    ...
    <div className="min-h-screen flex items-center justify-center bg-[#f5f5f5] from-gray-100 to-blue-100 px-4 py-10">
      <div className="w-full max-w-2xl bg-white p-8 rounded-2xl shadow-lg space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <h1 className="text-2xl font-bold text-gray-800">สมัครสมาชิกอาจารย์</h1>
        </div>
        ...
        {step === 1 && (
          <div className="space-y-4">
            <Input icon={<UserPlus />} name="full_name" placeholder="ชื่อ-นามสกุล" onChange={handleChange} />
            <Input icon={<Building2 />} name="faculty" placeholder="คณะ" onChange={handleChange} />
            <Input icon={<Briefcase />} name="department" placeholder="ภาควิชา" onChange={handleChange} />
            <Select name="position" value={form.position} onChange={handleChange} options={positionOptions} />
            ...
            <Input type="file" icon={<Upload />} accept="image/*" onChange={handleFileUpload} />
            {form.profile_picture_url && (
              <img src={form.profile_picture_url} alt="preview" className="h-24 w-24 rounded-full object-cover mx-auto border" />
            )}
<button
  disabled={loading}
  onClick={() => setStep(2)}
...
>
  ...
  ดำเนินการสมัคร (ถัดไป)
            ...
            <button onClick={() => setStep(1)} className="text-sm text-gray-500 hover:underline flex items-center justify-center gap-1">
