...
import { User, GraduationCap, BarChart2, ArrowRight } from "lucide-react";
...
function mapProfessorStudentToStudentWithSkills(
	stu: ProfessorStudent
): StudentWithSkills {
	return {
		...stu,
		id: stu.id ?? "", // Ensure id is always a string
		student_code: stu.student_id ?? "", // or map from another field if needed
		percent: stu.percent ?? 0,
		full_name: stu.full_name ?? "",
		year: stu.year ?? 1,
		curriculum_name: stu.curriculum_name ?? "",
		skills: Array.isArray(stu.skills)
			? (stu.skills.filter(
					(s) =>
						typeof s === "object" &&
						s !== null &&
...
export default function ProfessorDashboard() {
	...
	useEffect(() => {
		...
		getStudentsWithSkillsByProfessor(userId)
			.then((data: ProfessorStudent[]) =>
				setStudents(data.map(mapProfessorStudentToStudentWithSkills))
			)
			.catch(() => setStudents([]))
								...
								<ul className="flex flex-wrap gap-2">
									{stu.skills.length > 0 ? (
										stu.skills.map((s) => (
											<li
												key={s.skill_id}
												className={`rounded-full px-3 py-0.5 text-xs ${getSkillClass(
													s.status
												)}`}
											>
												{s.name_th}
