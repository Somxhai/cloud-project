...
export default function SkillManagementPage() {
  ...
  const handleAdd = async () => {
    ...
    setSavingAdd(true);
    try {
      const created = await createSkill({ ...form, description: form.description ?? '' });
  ...
  const toggleActive = async (skill: Skill) => {
    const updated = await updateSkill({ ...skill, is_active: !skill.is_active });
      ...
      {viewSkills.length === 0 ? (
        ...
        <ul className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {viewSkills.map((s) => {
            ...
            return (
              <li key={s.id} className="rounded-xl bg-white p-4 shadow-sm space-y-3">
                {/* top row */}
                <div className="flex items-start justify-between">
                  <h3 className="text-sm font-semibold">{s.name_th}</h3>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-md space-y-6 rounded-xl bg-white p-6 shadow-lg">
            <header className="flex items-center justify-between">
              <h2 className="text-lg font-semibold">เพิ่มทักษะ</h2>
