...
export default function ActivityDetailPage() {
	...
	const confirmed = approved.filter(
		(p) => p.confirmation_status === 1 && p.attended === null
	);
	const completed = participants.filter(
		(p) => p.status === 3 && p.attended === true
	);
	const notattending = participants.filter((p) => p.attended === false);
	const readOnly =
		!!activity &&
		(activity.is_published || [1, 2, 3].includes(activity.status));
	function StatsCard({
		pending,
		approved,
		confirmed,
	...
	const updateStatus = async (
		...
		try {
			...
			setActivity((prev) =>
				prev ? { ...prev, amount: newAmount } : prev
			);
		} catch (e) {
	...
	function SummaryCard({
		...
		const avg = (key: keyof ActivityEvaluation) =>
			(
				evaluations.reduce((s, e) => s + Number(e[key] || 0), 0) /
				evaluations.length
			).toFixed(1);
		return (
			<div className="grid gap-4 rounded-2xl bg-white p-6 shadow-sm sm:grid-cols-3 lg:grid-cols-6">
				...
				<Score
					title="คะแนนรวม"
					value={avg("score_overall")}
					highlight
				/>
				<div className="col-span-full text-xs text-gray-500">
					จำนวนการประเมิน {evaluations.length} ครั้ง • ผู้เข้าร่วมจริง{" "}
					{participants} คน
				</div>
			</div>
		);
	...
	const confirmOpenDate = () => {
		...
		event.setDate(
			event.getDate() - (activity.confirmation_days_before_event ?? 3)
		);
					...
					<div className="space-y-4">
						...
						<ul className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 text-sm text-gray-700">
							...
							<li className="flex items-start gap-2">
								...
								/>
								ปิดรับสมัคร:{" "}
								{ThaiDate(activity.registration_deadline)}
							</li>
							<li className="flex items-start gap-2">
								<Users2
									size={16}
							...
							<div className="flex items-center gap-2">
								...
								<input
									...
									onChange={(e) =>
										setActivity({
											...activity,
											confirmation_days_before_event:
				...
				<section className="space-y-6">
					...
					<div className="flex justify-between items-center">
						...
						<button
							onClick={async () => {
								setReloading(true);
								const refreshed = await getActivityParticipants(
									activityId
								);
								setParticipants(refreshed);
								setReloading(false);
					...
					{editableSkills.map((s, idx) => (
						...
						>
							...
							<input
								...
								value={s.note || ""}
								onChange={(e) =>
									onSkillChange(idx, "note", e.target.value)
								}
							...
							>
								...
								{isOpen && (
									<div className="grid grid-cols-2 gap-4 p-6 text-sm text-gray-700 border-t">
										...
										<Info
											label="ผู้บรรยาย"
											value={e.score_speaker}
										/>
										<Info
											label="ความน่าสนใจ"
											value={e.score_interest}
										/>
										<Info
											label="เนื้อหา"
											value={e.score_content}
										/>
										<Info
											label="การนำไปใช้"
											value={e.score_applicability}
										/>
										<Info
											label="คะแนนรวม"
			...
			{modalOpen && selectedStudent && (
				<Modal onClose={() => setModalOpen(false)}>
					<h2 className="mb-4 text-lg font-bold text-gray-800">
						ยืนยันทักษะให้กับ {selectedStudent.name}
					</h2>
					...
					<div className="space-y-4 max-h-[60vh] overflow-auto pr-2">
						{editableSkills.map((s, idx) => (
							<div
								key={idx}
								className="space-y-2 rounded bg-gray-50 p-4 text-sm"
							>
...
// 			✅ อนุมัติแล้ว: <b>{approved}</b> คน | 🛎️ ยืนยันแล้ว:{" "}
// 			<b>{confirmed}</b> คน
// 		</div>
// 	);
// }
	...
	return (
		<div className="space-y-2">
			<h3 className="text-lg font-semibold text-gray-800 flex items-center gap-2">
				...
				{title.includes("ยืนยันเข้าร่วม") && (
					<CalendarCheck2 size={16} />
				)}
				{title.includes("เข้าร่วมกิจกรรมเรียบร้อย") && (
					<Brain size={16} />
				)}
				{title.includes("ผู้ที่ไม่มาเข้าร่วมกิจกรรม") && (
					<XCircle size={16} />
				)}
			...
			{items.length > 0 && (
				<div className="overflow-x-auto">
					<table
						className={`w-full table-auto border text-sm rounded-lg overflow-hidden ${variantClass}`}
					>
						<thead>
...
function ActionButton({ onClick, label, variant, Icon }: ActionButtonProps) {
	...
	return (
		<button
			onClick={onClick}
			className={`inline-flex items-center gap-1 ${color} text-sm`}
		>
			<Icon size={14} /> {label}
		</button>
	);
