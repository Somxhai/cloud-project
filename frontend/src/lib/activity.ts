...
export const addSkillsToActivity = async (
	...
	const res = await fetch(
		`${process.env.NEXT_PUBLIC_API_BASE_URL}/activity/${activityId}/skill-ids`,
		{
			method: "PUT",
...
export async function getActivityDetail(
	id: string
): Promise<ActivityWithSkills> {
	const res = await fetch(
		`${process.env.NEXT_PUBLIC_API_BASE_URL}/activity/detail/${id}`,
		{
			headers: await getAuthHeaders(), // ✅ แนบ token
			method: "GET",
			cache: "no-store",
...
export async function getOpenActivities(): Promise<ActivityWithSkills[]> {
	const res = await fetch(
		`${process.env.NEXT_PUBLIC_API_BASE_URL}/activity/open`,
		{
			method: "GET",
			cache: "no-store",
			headers: await getAuthHeaders(), // ✅ แนบ token
 ...
 */
export async function getAllActivitiesWithSkills(): Promise<
	ActivityWithSkills[]
> {
	const res = await fetch(
		`${process.env.NEXT_PUBLIC_API_BASE_URL}/activity/with-skills`,
		{
			method: "GET",
			cache: "no-store",
			headers: await getAuthHeaders(), // ✅ แนบ token
...
export async function getParticipantsByActivityId(
	id: string
): Promise<StudentActivityWithStudentInfo[]> {
	const res = await fetch(
		`${process.env.NEXT_PUBLIC_API_BASE_URL}/activity/participants/${id}`,
		{
			method: "GET",
			cache: "no-store",
			headers: await getAuthHeaders(), // ✅ แนบ token
...
export async function updateActivityStatus(id: string, status: number): Promise<Activity> {
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/activity/status/${id}`, {
    method: 'PUT',
    headers: await getAuthHeaders(), // ✅ แนบ token
...
export async function recalculateAllStudentSkills(): Promise<{
	message: string;
}> {
	const res = await fetch(
		`${process.env.NEXT_PUBLIC_API_BASE_URL}/activity/recalculate-skill`,
		{
			method: "POST",
			headers: await getAuthHeaders(),
		}
...
export async function recalculateAllActivityAmount(): Promise<{
	message: string;
}> {
	const res = await fetch(
		`${process.env.NEXT_PUBLIC_API_BASE_URL}/activity/recalculate-amount`,
		{
			method: "POST",
			headers: await getAuthHeaders(),
		}
	...
	if (!res.ok)
		throw new Error("ไม่สามารถคำนวณจำนวนผู้เข้าร่วมกิจกรรมใหม่ได้");
	return res.json();
}
...
export async function saveActivityChanges(
	...
	await Promise.all([
		fetch(
			`${process.env.NEXT_PUBLIC_API_BASE_URL}/activity/${id}/confirm-days`,
			{
				method: "PUT",
				headers: await getAuthHeaders(),
...
export async function markStudentAbsent(studentId: string): Promise<void> {
	const res = await fetch(
		`${process.env.NEXT_PUBLIC_API_BASE_URL}/activity/mark-absent`,
		{
			method: "POST",
			headers: await getAuthHeaders(),
...
export async function getActivityParticipants(
	...
	const res = await fetch(
		`${process.env.NEXT_PUBLIC_API_BASE_URL}/activity/${id}/participants`,
		{
			headers: await getAuthHeaders(),
		}
	);
...
export async function updateStudentActivityStatus(
	...
	const res = await fetch(
		`${process.env.NEXT_PUBLIC_API_BASE_URL}/activity/student-activity/${studentActivityId}/status`,
		{
			method: "PUT",
			headers: await getAuthHeaders(),
			body: JSON.stringify({ status }),
...
export async function getActivitySkills(
	activityId: string
): Promise<ActivitySkill[]> {
	const res = await fetch(
		`${process.env.NEXT_PUBLIC_API_BASE_URL}/activity/${activityId}/skills`
	);
...
export async function deleteActivitySkill(
	activityId: string,
	skillId: string
): Promise<void> {
	const res = await fetch(
		`${process.env.NEXT_PUBLIC_API_BASE_URL}/activity/${activityId}/skill/${skillId}`,
		{
			method: "DELETE",
			headers: await getAuthHeaders(),
		}
...
export async function getActivityEvaluations(
	activityId: string
): Promise<ActivityEvaluation[]> {
	const res = await fetch(
		`${process.env.NEXT_PUBLIC_API_BASE_URL}/activity/${activityId}/evaluations`,
		{
			headers: await getAuthHeaders(),
		}
	);
...
export async function updateActivity(
	...
	const res = await fetch(
		`${process.env.NEXT_PUBLIC_API_BASE_URL}/activity/${activityId}`,
		{
			method: "PUT",
			headers: await getAuthHeaders(),
...
export async function addSkillsToStudent(
	...
	const res = await fetch(
		`${process.env.NEXT_PUBLIC_API_BASE_URL}/student/addStudentSkills`,
		{
			method: "POST",
			headers: await getAuthHeaders(),
...
export async function submitActivityEvaluation(payload: {
	...
	const res = await fetch(
		`${process.env.NEXT_PUBLIC_API_BASE_URL}/activity/evaluation`,
		{
			method: "POST",
			headers: await getAuthHeaders(),
			body: JSON.stringify(payload),
...
export async function updateConfirmationDays(activityId: string, days: number) {
	const res = await fetch(
		`${process.env.NEXT_PUBLIC_API_BASE_URL}/activity/${activityId}/confirm-days`,
		{
			method: "PUT",
			headers: await getAuthHeaders(),
			body: JSON.stringify({ days }),
...
export async function confirmStudentSkills(
	...
	const res = await fetch(
		`${process.env.NEXT_PUBLIC_API_BASE_URL}/activity/${activityId}/confirm-skills/${studentId}`,
		{
			method: "POST",
			headers: await getAuthHeaders(),
			body: JSON.stringify(skills),
...
export async function updateActivityPublish(id: string, pub: boolean) {
	const res = await fetch(
		`${process.env.NEXT_PUBLIC_API_BASE_URL}/activity/${id}/publish`,
		{
			method: "PUT",
			headers: await getAuthHeaders(),
...
export async function updateActivityStatus(id: string, status: number) {
	const res = await fetch(
		`${process.env.NEXT_PUBLIC_API_BASE_URL}/activity/${id}/status`,
		{
			method: "PUT",
			headers: await getAuthHeaders(),
			body: JSON.stringify({ status }),
...
export async function recalculateAmount(activityId: string): Promise<number> {
	const res = await fetch(
		`${process.env.NEXT_PUBLIC_API_BASE_URL}/activity/recalculate-amount/${activityId}`,
		{
			method: "POST",
			headers: await getAuthHeaders(),
		}
...
export async function updateAttendance({
	...
	const res = await fetch(
		`${process.env.NEXT_PUBLIC_API_BASE_URL}/student-activity/mark-attendance`,
		{
			method: "POST",
			headers: await getAuthHeaders(),
