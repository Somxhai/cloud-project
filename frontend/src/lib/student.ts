...
export async function getStudentActivities(
	...
	const res = await fetch(
		`${process.env.NEXT_PUBLIC_API_BASE_URL}/student/${studentId}/activities`,
		{
			cache: "no-store",
			headers: await getAuthHeaders(),
		}
...
export async function getStudentSkills(studentId: string): Promise<Skill[]> {
	const res = await fetch(
		`${process.env.NEXT_PUBLIC_API_BASE_URL}/student/${studentId}/skills`,
		{
			cache: "no-store",
			headers: await getAuthHeaders(),
		}
...
export async function joinActivity(student_id: string, activity_id: string): Promise<StudentActivity> {
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/student/join`, {
    method: 'POST',
    headers: await getAuthHeaders(),
...
> {
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/student/detail/${studentId}`, {
    cache: 'no-store',
    headers: await getAuthHeaders(),
  });
...
export async function getCompletedActivitiesWithSkills(studentId: string): Promise<ActivityWithSkills[]> {
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/student/completed/${studentId}`, {
    cache: 'no-store',
    headers: await getAuthHeaders(),
  });
...
export async function getStudentActivityStatus(student_id: string, activity_id: string): Promise<StudentActivity | null> {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_API_BASE_URL}/student/activity-status?student_id=${student_id}&activity_id=${activity_id}`,
    {
      headers: await getAuthHeaders(),
    }
  );
...
export async function updateStudentActivityStatus(
	...
	const res = await fetch(
		`${process.env.NEXT_PUBLIC_API_BASE_URL}/student/update-status`,
		{
			method: "PUT",
			headers: await getAuthHeaders(),
...
export async function getStudentFullDetail(studentId: string) {
	const res = await fetch(
		`${process.env.NEXT_PUBLIC_API_BASE_URL}/student/${studentId}/detail`,
		{
			headers: await getAuthHeaders(),
		}
	);
...
export async function getCompletedActivitiesWithSkills(studentId: string) {
	const res = await fetch(
		`${process.env.NEXT_PUBLIC_API_BASE_URL}/student/${studentId}/activities/completed`,
		{
			headers: await getAuthHeaders(),
		}
	);
...
export async function getStudentActivityStatus(
	studentId: string,
	activityId: string
) {
	const res = await fetch(
		`${process.env.NEXT_PUBLIC_API_BASE_URL}/student-activity/status?studentId=${studentId}&activityId=${activityId}`,
		{
			headers: await getAuthHeaders(),
		}
	);
...
export async function joinActivity(
	...
	const res = await fetch(
		`${process.env.NEXT_PUBLIC_API_BASE_URL}/student-activity/join`,
		{
			method: "POST",
			headers: await getAuthHeaders(),
			body: JSON.stringify({
...
export async function getMyActivities(studentId: string): Promise<StudentActivityWithActivityInfo[]> {
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/student/my-activities?studentId=${studentId}`);
...
export async function confirmAttendance(
	...
	const res = await fetch(
		`${process.env.NEXT_PUBLIC_API_BASE_URL}/student-activity/${studentId}/confirm`,
		{
			method: "PUT",
			headers: await getAuthHeaders(),
...
export async function getStudentProgress(
	...
	const res = await fetch(
		`${process.env.NEXT_PUBLIC_API_BASE_URL}/progress/student/${studentId}`,
		{
			cache: "no-store",
			headers: await getAuthHeaders(),
		}
...
export async function getStudentActivityHistory(
	...
	const res = await fetch(
		`${process.env.NEXT_PUBLIC_API_BASE_URL}/progress/student/${studentId}/activity-history`,
		{
			cache: "no-store",
			headers: await getAuthHeaders(),
		}
...
export async function getMyActivities(
	...
	const res = await fetch(
		`${process.env.NEXT_PUBLIC_API_BASE_URL}/student/my-activities?studentId=${studentId}`,
		{
			headers: await getAuthHeaders(),
		}
	);
...
export async function submitFeedback(
	...
	const res = await fetch(
		`${process.env.NEXT_PUBLIC_API_BASE_URL}/student/${studentId}/activity/${activityId}/feedback`,
		{
			method: "POST",
			headers: await getAuthHeaders(),
		}
...
export const checkStudentCodeExists = async (
	...
	const res = await fetch(
		...
		}/public/check-code?student_code=${encodeURIComponent(studentCode)}`,
		{
			headers: await getAuthHeaders(),
		}
	);
