...
// GET /curriculum/:id
export async function getCurriculumDetail(id: string): Promise<CurriculumDetail> {
  const res = await fetch(`${BASE_URL}/curriculum/${id}`, { cache: 'no-store' });
...
export async function deleteCurriculum(id: string): Promise<void> {
  const res = await fetch(`${BASE_URL}/curriculum/${id}`, {
    method: 'DELETE',
  });
...
export async function updateCurriculumSkills(id: string, skills: CurriculumSkillInput[]): Promise<void> {
  const res = await fetch(`${BASE_URL}/curriculum/${id}/skills`, {
    method: 'PUT',
...
/* ------------------------------ สร้างหลักสูตรใหม่ ------------------------------ */
// POST /curriculum
export async function createCurriculum(payload: { name: string; description?: string }): Promise<Curriculum> {
